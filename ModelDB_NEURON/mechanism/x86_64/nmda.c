...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 9, 3);
 	...
 	ivoc_help("help ?1 NMDA /home/bandi/workspace/UCL/Poirazi2003/ModelDB_NEURON/mechanism/x86_64/nmda.mod\n");
...
static double _nrn_current(double _v){double _current=0.;v=_v;{ {
   g = ( gmax * R ) / ( 1.0 + eta * mag * exp ( - ( gamma * v ) ) ) ;
   i = g * ( v - Erev ) ;
   }
 _current += i;
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  release();
 if(error){fprintf(stderr,"at line 145 in file nmda.mod:\n	SOLVE release\n"); nrn_complain(_p); abort_run(error);}
